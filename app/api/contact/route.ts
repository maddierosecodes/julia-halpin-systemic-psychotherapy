...
export async function POST(request: Request) {
  try {
    ...
    // Verify Turnstile
    devLog('Verifying Turnstile token...');
    const isValidCaptcha = await verifyTurnstile(captchaToken);
    if (!isValidCaptcha) {
      devLog('Turnstile verification failed');
      return NextResponse.json(
        ...
        return NextResponse.json(
          {
            ...
            debug: {
              emailContent,
              to: 'julia.halpin@familyandcoupletherapyservices.co.uk',
              from: 'julia.halpin@familyandcoupletherapyservices.co.uk',
    ...
    // Create transporter with ImprovMX settings
    const transporter = nodemailer.createTransport({
      host: 'smtp.improvmx.com',
      port: 587,
      secure: false, // TLS
      auth: {
        user: 'julia.halpin@familyandcoupletherapyservices.co.uk',
